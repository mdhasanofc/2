...
let currentUser=null, cameras=[], favorites=new Set(), activeFilter='all';
...
function setAuthTab(mode){$$('[data-tab]').forEach(b=>b.classList.toggle('active',b.dataset.tab===mode));$('#signupForm').classList.toggle('hidden',mode!=='signup');$('#loginForm').classList.toggle('hidden',mode!=='login');$('#authTitle').textContent=mode==='signup'?'Create your account':'Welcome back';$('#authSub').textContent=mode==='signup'?'Use your email and verify it from your inbox.':'Log in with your verified account.';setMsg('#signupMsg','');setMsg('#loginMsg','')}
function openAuth(mode='signup'){setAuthTab(mode);openModal(authModal)}
...
async function loadCameras(){
  ...
  if(error){$('#camGrid').innerHTML='<div class="empty">Could not load cameras. Please try again.</div>';return}
  cameras=data||[]; renderCameras();
}

function renderCameras(){
  if(!currentUser){$('#camGrid').innerHTML='<div class="empty">Sign in to view live cameras.</div>';return}
  ...
  $('#camGrid').innerHTML=list.length?list.map(c=>`<article class="card"><div class="thumb"><img src="${c.thumbnail_url||''}" alt="${c.title}" loading="lazy"><span class="live badge"><i></i> LIVE</span><button class="fav ${favorites.has(String(c.id))?'active':''}" data-fav="${c.id}">${favorites.has(String(c.id))?'♥':'♡'}</button></div><div class="card-body"><h3>${c.title}</h3><p>${c.location}</p><button class="btn primary full" data-watch="${c.id}">Watch live</button></div></article>`).join(''):'<div class="empty">No cameras found.</div>';
...
function openCamera(id){
  ...
  $('#playerTitle').textContent=cam.title; $('#playerLocation').textContent=cam.location;
...
async function refreshAuthUI(){
  ...
  $('#startBtn').textContent=signed?'Open dashboard':'Create free account';
  $('#authNotice').className='status '+(signed?'ok':'warn'); $('#authNotice').textContent=signed?'Secure account session active.':'Email verification is required before account access.';
...
$('#previewPlay').onclick=()=>currentUser?openCamera(cameras[0]?.id):openAuth('login');
